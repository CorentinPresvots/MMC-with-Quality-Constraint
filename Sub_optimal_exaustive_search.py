...
class Encode_one_window(Model_Encoder, Residual_Encoder):
    ...
    def f(self,id_model,r, nx,metric,quality):
        """
        ...
        for id_residual_test in self.best_Residual_used:
            ...
            x_residual_test, code_residual_test, kr_test, nb_sym_test = self.enc_residual(id_residual_test, r,metric,quality)
            
            if self.best_Residual_used[id_residual_test]["name"]=="DCT+BPC" or self.best_Residual_used[id_residual_test]["name"]=="DWT+BPC":
                self.max_size_Nx += 1
            ...
            if nr_test<nr_min and quality_residual_test <= quality:
                ...
                # no need to test other methods because bypass second stage is the less method to encode
        ...
        return  quality_residual,id_residual, kr, nb_sym, code_residual, x_residual
    ...
    def get_n_max(self,x,metric,quality):
        ...
        for id_residual_test in self.best_Residual_used:
            ...
            # Encode the residual using the current compression method
            x_residual_test, code_residual_test, kr_test, nb_sym_test = self.enc_residual(id_residual_test,x,metric,quality)
        ...
        #print("x","quality_residual",quality_residual,nr_min)
    ...
    def get_n_min_m(self,id_model,metric,quality):
        ...
        _,_,x_model_tilde=self.enc_model(id_model, self.nb_max_bit_theta*len(self.best_Model_used[id_model]["w theta"]))
        
        x_residual=(self.best_Model_used[id_model]["xn"]-x_model_tilde)*2**(self.best_Model_used[id_model]["kx"])
        ...
        for id_residual_test in self.best_Residual_used:
            ...
            # Encode the residual using the current compression method
            x_residual_test, code_residual_test, kr_test, nb_sym_test = self.enc_residual(id_residual_test,x_residual,metric,quality)
        ...
        #print("x-xmodel","quality_residual",quality_residual,nr_min)       
    ...
    def MMC_enc(self, x, x_previous,metric,quality):
        ...
        n_tot=np.infty#self.get_n_max(x,metric,quality)
            
            
        fact=2
        max_rate_per_model=70
        # Define a range of bit budgets for model encoding
        Nx = [i for i in range(0,max_rate_per_model, fact)]
    
        
        # Iterate over possible bit allocations for models
        for n_x in Nx:
            
            if n_x<=max_rate_per_model:
                quality_model = np.inf  # Initialize the best SNR for the current model
                for id_model_test in self.best_Model_used:
                    ...
                    if self.nb_min_bit_theta * len(self.best_Model_used[id_model_test]["w theta"]) <= n_x <= 2 ** self.best_Model_used[id_model_test]["n nx"] - 1:
                        ...
                        quality_model_test = get_quality(self.best_Model_used[id_model_test]["xn"], x_model_test, "SNR")
                ...
                ##################### quality criterion normalize
                if metric== "RMSE" :
                    quality_n=quality*2**(-self.best_Model_used[id_model_test]["kx"])
                elif metric=="MSE":
                    quality_n=quality*2**(-2*self.best_Model_used[id_model_test]["kx"])
                elif metric== "SNR":
                    quality_n=quality
                else : 
                ...
                #print("quality r target",quality_r_target)
                quality_residual,id_residual, kr, nb_sym, code_residual, x_residual=self.f(id_model,r,nx_model,metric,quality_r_target)
                
                
                n_r=len(code_residual)
            
                
                n_tot_test=self.nm+self.best_Model_used[id_model]["n kx"]+self.best_Model_used[id_model]["n nx"]+n_x+self.nl+self.best_Residual_used[id_residual]["n kr"]+self.best_Residual_used[id_residual]["n nr"]+n_r
                
                #print(n_tot_test,n_tot,quality_residual,quality_r_target)
                if n_tot_test<n_tot and quality_residual<= quality_r_target :
                    n_tot=n_tot_test
                    ...
                    max_rate_per_model = min(max_rate_per_model,n_tot) 
