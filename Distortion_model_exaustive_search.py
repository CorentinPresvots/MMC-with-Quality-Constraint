...
class Encode_one_window(Model_Encoder, Residual_Encoder):
    ...
    def get_m_nx_nr(self, metric, quality, x, kx):
        ...
        for id_model in self.Model_used:
            ...
            #print("m",self.best_Model_used[id_model]["name"],"MSE_test",np.sqrt(MSE_test*2**(2*kx)),np.sqrt(MSE_target*2**(2*kx)))
    ...
    def MMC_enc(self, x, x_previous, metric, quality):
        ...
        for the first stage. Then, it performs an exhaustive search to find the optimal nx value. 
        ...
        """
        # Step 1: Initialize the encoding process (prepares internal variables, resets states, etc.)
        self.ini_MMC_enc()
    
        # Step 2: Compute scaling factors for the current and previous signals. 
        # These factors are used to normalize the signals for consistent parameter estimation.
        kx, kx_previous = self.get_header(x, x_previous)
        
        # Step 3: Estimate initial model parameters (e.g., sine or polynomial coefficients) 
        # based on the normalized input signal. The parameters represent the first-stage approximation.
        self.get_theta()
        
        # Step 4: Determine a subset of candidate models and their corresponding bit allocations 
        # for the first stage, based on the provided quality constraint.
        id_models = self.get_m_nx_nr(metric, quality, x, kx)
        
        # Initialize a variable to track the minimum total encoding cost.
        n_tot = np.infty
        
        # Step 5: Iterate through each candidate model to refine the bit allocation.
        for id_model_test in id_models:
            # Use the best bit allocation estimated so far as the initial guess.
            n_x_opt = self.best_Model_used[id_model_test]["nx"]
            
            # Define the range of bitrates (number of bits per parameter) to test for the model.
            min_rate_per_parameter = np.max([n_x_opt - self.delta_nx, 
                                             self.nb_min_bit_theta * len(self.best_Model_used[id_model_test]["w theta"])])
            max_rate_per_parameter = np.min([n_x_opt + self.delta_nx,
                                             np.min([self.nb_max_bit_theta * len(self.best_Model_used[id_model_test]["w theta"]),
                                                     2**self.best_Model_used[id_model_test]["n nx"] - 1])])
            
            # Adjust step size for searching bit allocations.
            fact = 2
            Nx = [i for i in range(min_rate_per_parameter, max_rate_per_parameter + fact, fact)]
            
            # Evaluate each possible bit allocation in the specified range.
            for n_x in Nx:
                if n_x <= max_rate_per_parameter:
                    # Encode the model parameters and evaluate residual compression methods for the current bit allocation.
                    quality_model, quality_residual, theta_tilde, code_model, x_model, id_residual, \
                    kr, nb_sym, code_residual, x_residual = self.f(id_model_test, n_x, metric, quality)
    
                    # Calculate the total encoding cost for this configuration.
                    n_tot_test = (self.nm + self.best_Model_used[id_model_test]["n kx"] + 
                                  self.best_Model_used[id_model_test]["n nx"] + len(code_model) + self.nl +
                                  self.best_Residual_used[id_residual]["n kr"] +
                                  self.best_Residual_used[id_residual]["n nr"] + len(code_residual))
    
                    # If this configuration uses fewer total bits while meeting the quality constraint, store it as the best configuration.
                    if n_tot_test < n_tot:
                        n_tot = n_tot_test
                        ...
                        # Update the scaling factor or key parameter for the residual compression.
                        self.best_Residual_used[id_residual]["kr"] = kr
                        self.best_model_used = id_model_test
                        self.best_residual_used = id_residual
                        
                        # Refine the range for subsequent tests to focus on configurations with lower total cost.
                        max_rate_per_parameter = min(max_rate_per_parameter, n_tot)
