...
class Encode_one_window(Model_Encoder, Residual_Encoder):
    ...
    def MMC_enc(self, x, x_previous, metric, quality):
        """
        Perform an exhaustive search for Model-Residual Compression (MMC) encoding.
    
        This function evaluates all possible combinations of models and bit allocations
        to determine the configuration that minimizes the total number of bits used while 
        maintaining a specified quality.
        ...
        # Step 4: Determine the maximum number of bits allowed based on the target quality
        ...
        for id_model_test in self.best_Model_used:
            ...
            n_min = self.get_n_min_m(id_model_test, metric, quality) + self.n_kx + self.n_kr + self.nm + self.nl + self.n_sym + 1
            ...
            # Generate a list of candidate bitrates to test
            step = 2
            Nx = [i for i in range(min_rate_per_parameter, max_rate_per_parameter + step, step)]
    
            # Step 6: Evaluate each candidate bitrate for the current model
            for n_x in Nx:
                if n_x <= max_rate_per_parameter:
                    # Perform the encoding with the current model and bitrate
                    ...
                    # Calculate the total number of bits used by this configuration
                    n_tot_test = (
                        self.nm +
                        self.best_Model_used[id_model_test]["n kx"] +
                        self.best_Model_used[id_model_test]["n nx"] +
                        len(code_model) +
                        self.nl +
                        self.best_Residual_used[id_residual]["n kr"] +
                        self.best_Residual_used[id_residual]["n nr"] +
                        len(code_residual)
                    )
    
                    # If this configuration uses fewer bits than the previous best, update the best configuration
                    if n_tot_test < n_tot:
                        n_tot = n_tot_test
                        ...
                        # Update the upper limit for bitrates based on the current best configuration
                        max_rate_per_parameter = min(
                            max_rate_per_parameter,
                            max([min_rate_per_parameter, n_tot - n_min])
                        )
