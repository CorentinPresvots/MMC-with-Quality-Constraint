...
# Method 1: Exhaustive Search (~100 times O(Nlog(N)))
# - This method is the most computationally expensive, running exhaustive tests on all possible configurations.
# - Compression performance is the highest among the options (rate mean 91.4 bits/ window on data_S2 at RMSE_max=200 V).

# Method 2: Golden Section Search (~74.2 times O(Nlog(N)))
...
# - Compression performance is only slightly lower (rate mean 92.6 bits/ window at RMSE_max=200 V).

# Method 3: Sub-optimal Exhaustive Search (~34.5 times O(Nlog(N)))
# - A simplified version of exhaustive search that tests fewer configurations.
# - While less computationally demanding, it still offers decent compression performance (rate mean 91.7 bits/ window at RMSE_max=200 V).

#Method 4: Distortion Model Exhaustive Search (~16 times O(Nlog(N)))
#- Estimation of the model and bit distributions between two stages, using a variant of exhaustive search.
#- This approach reduces computational complexity further, but with a slight impact on compression quality (rate mean 94.6 bits/ window at RMSE_max=200 V).

#Method 5: Distortion Model Golden Section Search (~10.5 times O(Nlog(N)))
...
#- This method is the fastest among all options but results in the lowest compression performance (rate mean 96.9 bits/ window at RMSE_max=200 V).
...
for id_signal in range(nb_signal):
    ...
    for phase in range(nb_phase):
        ...
        for w in range(nb_w):
            ...
            dict_MMC_enc[phase].MMC_enc(x_test, x_previous,metric,quality)
